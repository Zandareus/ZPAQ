...
namespace libzpaq {
	...
	public:
		AES_CTR(const char* key, int keylen, const char* iv = nullptr);
	...
	public:
		...
		bool findBlock(double* memptr = nullptr);
		void hcomp(Writer* out2) { z.write(out2, false); }
		bool findFilename(Writer* = nullptr);
		void readComment(Writer* = nullptr);
		...
		void readSegmentEnd(char* sha1string = nullptr);
	...
	public:
		Encoder(ZPAQL& z, int size = 0) :
			out(nullptr), low(1), high(0xFFFFFFFF), pr(z) {}
		void init();
	...
	private:
		...
		void syntaxError(const char* msg, const char* expected = nullptr); // error()
	...
	class Compressor {
	public:
		Compressor() : enc(z), in(nullptr), state(INIT), verify(false) {}
		...
		void startBlock(const char* config,     // ZPAQL source code
			...
			Writer* pcomp_cmd = nullptr); // retrieve preprocessor command
		void setVerify(bool v) { verify = v; }    // check postprocessing?
		void hcomp(Writer* out2) { z.write(out2, false); }
		bool pcomp(Writer* out2) { return pz.write(out2, true); }
		void startSegment(const char* filename = nullptr, const char* comment = nullptr);
		void setInput(Reader* i) { in = i; }
		void postProcess(const char* pcomp = nullptr, int len = 0);  // byte code
		bool compress(int n = -1);  // n bytes, -1=all, return true until done
		void endSegment(const char* sha1string = nullptr);
		char* endSegmentChecksum(int64_t* size = nullptr, bool dosha1 = true);
	...
	public:
		...
		StringBuffer(size_t n = 0) :
			p(nullptr), al(0), wpos(0), rpos(0), limit(size_t(-1)), init(n > 128 ? n : 128) {}
	...
	void compress(Reader* in, Writer* out, const char* method,
		const char* filename = nullptr, const char* comment = nullptr, bool dosha1 = true);
